...
| **AI 模型** | 智谱 GLM-4-Flash | 国产大模型，用于安全分析对话 |
...
│  │   Three.js + WebGL  │  │    GLM-4-Flash + MCP Tools │ │
│  └─────────┬───────────┘  └────────────┬───────────────┘ │
│            │  WebSocket + REST API       │                 │
├────────────┼────────────────────────────┼─────────────────┤
│            ▼                            ▼                 │
...
```
用户提问 → AI 大模型(GLM) → 理解意图 → 选择合适的 MCP 工具 → 执行 → 返回结果 → AI 分析结果
```
...
当 GLM API 不可用时，系统仍能工作——直接格式化工具结果为可读文本返回给用户，确保功能不中断。
...
| **回退容错** | 每个模块都有回退机制：nmap→scapy→static，MCP→直接服务调用，GLM→格式化回退 |
...
| **AI 依赖外部 API** | 中 | GLM-4-Flash 通过云端 API 调用，离线环境无法使用。虽然有回退机制，但 AI 分析能力会降级 |
