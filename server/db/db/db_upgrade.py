...
from server.db.compat import mylog, Logger
# messaging.in_app replaced
...
def migrate_timestamps_to_utc(sql) -> bool:
    ...
    try:
        ...
        mylog("none", "[db_upgrade] UTC timestamp migration completed")
        return True
...
def ensure_cyberclaw_tables(sql):
    """创建 CyberClaw 扩展表（不在原始 schema 中）"""
    try:
        sql.execute("""CREATE TABLE IF NOT EXISTS security_events (
        ...
        sql.execute("CREATE INDEX IF NOT EXISTS idx_secev_source ON security_events(source_type)")
        sql.execute("CREATE INDEX IF NOT EXISTS idx_secev_severity ON security_events(severity)")
        sql.execute("CREATE INDEX IF NOT EXISTS idx_secev_ts ON security_events(timestamp)")
        ...
        # 为 Devices 表添加 CyberClaw 安全字段（如果不存在）
        cyberclaw_columns = [
            ("devStatus", "TEXT DEFAULT 'secure'"),
            ("devDiscoveryMethod", "TEXT DEFAULT 'config'"),
            ("devOpenPorts", "TEXT DEFAULT '[]'"),
            ("devProtocols", "TEXT DEFAULT '[]'"),
            ("devOsGuess", "TEXT DEFAULT ''"),
            ("devModel", "TEXT DEFAULT ''"),
            ("devNotes", "TEXT DEFAULT ''"),
            ("devFirmwareVersion", "TEXT DEFAULT ''"),
            ("devSerialNumber", "TEXT DEFAULT ''"),
            ("devSwitchPort", "TEXT DEFAULT ''"),
            ("devRole", "TEXT DEFAULT 'target'"),
            ("devPos", "TEXT DEFAULT '[]'"),
            ("devIconSource", "TEXT DEFAULT 'CONFIG'"),
            ("devTypeSource", "TEXT DEFAULT 'CONFIG'"),
        ]
        for col_name, col_type in cyberclaw_columns:
            try:
                sql.execute(f'PRAGMA table_info("Devices")')
                actual = [row[1] for row in sql.fetchall()]
                if col_name not in actual:
                    sql.execute(f'ALTER TABLE "Devices" ADD "{col_name}" {col_type}')
            except Exception:
                pass

        mylog("debug", "[db_upgrade] CyberClaw tables ensured")
        return True
    except Exception as e:
        mylog("error", f"[db_upgrade] CyberClaw tables error: {e}")
        return False
