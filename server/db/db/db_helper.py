...
from server.db.helper import if_byte_then_to_str, get_setting_value  # noqa: E402 [flake8 lint suppression]
from server.db.compat import mylog, Logger
from server.db.const import NULL_EQUIVALENTS_SQL  # noqa: E402 [flake8 lint suppression]
...
def get_device_conditions():
    ...
    conditions = {
        ...
        "unknown": f"WHERE {base_active} AND ({NULL_EQUIVALENTS_SQL.replace('COL', 'devName')})",
        "known": f"WHERE {base_active} AND NOT ({NULL_EQUIVALENTS_SQL.replace('COL', 'devName')})",
